  ...
  const UploadModal: React.FC<UploadModalProps> = ({ isUpload, setIsUpload }) => {
    ...
    const handleUploadImage = async () => {
      ...
      try { 
        ...
        /* 
          TODO: Persist the images in the hard disk using electron-store package

          The images must be stored in a folder to keep the images organized and grouped depends on when and where the images are uploaded.
        */
       if(isLoading === false) {
        setTimeout(() => {
          navigate("/system")
          ...
          >
            ...
            > 
              ...
              <p className={`${theme === 'dark' ? 'text-dark' : 'text-white'} font-regular`}>
                Upload raw CT Scan Images with <span className="font-bold italic">DICOM, JPG, or PNG</span> format
              </p>
                      ...
                      >   
                        <div className="flex gap-4 items-center">
                          ...
                          <div className="flex flex-col gap-1">
                            <p className={`${theme === 'dark' ? 'text-dark' : 'text-white'} font-semibold`}>
                              {image.name}
                            </p>
                            <p className={`${theme === 'dark' ? 'text-dark_g' : 'text-white'} text-xs`}>
                        ...
                        <button
                          onClick={() => removeSpecificFile(image.name)}
                        >
                          <IoMdClose size={20} />
                        </button>
                      </div>
